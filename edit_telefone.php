...
<div style='width:600px;  margin:auto;  border:2px solid #BBB;  padding:0px 10px 30px;'>
	...
	<?php
		...
		if($erro){   print"Não é possivel continuar!<br/>".$erro;   }
					...
					$valor=db_query("SELECT telefone   FROM clientes   WHERE id='".$_GET['cliente']."'");
					$valor=mysql_fetch_row($valor['result']);
					$_ENV['form_fields']['Telefone']['value']=$valor[0];
					unset($valor);
					if(isset($_POST['formulario']) && $_POST['formulario']==="enviado"){   unset($_POST['formulario']);
						...
						if($erros){
							...
							//ATUALIZA O CLIENTE
							$db_op=db_query("UPDATE `clientes`   SET `telefone`='".$_ENV['form_fields']['Telefone']['value']."'   WHERE id='".$_GET['cliente']."'");
